...
@section('content')
    <div class="min-h-screen bg-black text-white pb-10 pt-20 w-full justify-center">
        <div class="max-w-6xl mx-auto p-6 rounded-lg shadow-md">
            <div class="flex justify-between items-center mb-6">
                ...
                <a href="{{ route('modul.create') }}"
                    class="px-4 py-2 bg-green-600 hover:bg-green-700 rounded text-white font-semibold">+ Tambah Modul</a>
            </div>
                ...
                <form action="{{ route('modul.index') }}" method="GET"
                    ...
                    <div class="relative w-full md:flex-1">
                        <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                            <svg class="w-5 h-5 text-black opacity-60" xmlns="http://www.w3.org/2000/svg" fill="none"
                    ...
                    <div class="flex flex-col md:flex-row gap-3 w-full md:w-auto">
                        ...
                        <div class="relative w-full md:w-60">
                            <select name="sort"
                                ...
                                <option value="nama_modul" {{ request('sort') == 'nama_modul' ? 'selected' : '' }}>Judul Modul
                                </option>
                                <option value="level_id" {{ request('sort') == 'level_id' ? 'selected' : '' }}>Kategori
                                </option>
                                <option value="created_by" {{ request('sort') == 'created_by' ? 'selected' : '' }}>Pembuat
                                </option>
                                <option value="updated_at" {{ request('sort') == 'updated_at' ? 'selected' : '' }}>Terakhir
                                    Diperbarui</option>
...
@section('sidebar')
    ...
    <div class="flex flex-col gap-8 h-full w-full pl-8">
        <a href="{{ route('modul.index') }}">
            <div
                class="px-4 py-3 rounded-xl w-[80%] {{ $currentRoute == 'modul.index' ? 'bg-amber-300 text-black' : 'text-white hover:bg-amber-300 hover:text-black transition-all' }}">
                <h2 class="text-xl font-bold m-0 p-0">Daftar Modul</h2>
            </div>
        </a>

        <div
            class="px-4 py-3 rounded-xl w-[80%] {{ request()->is('dashboard-admin-profile') ? 'bg-amber-300 text-black' : 'text-white hover:bg-amber-300 hover:text-black transition-all' }}">
            <h2 class="text-xl font-bold m-0 p-0">Daftar User</h2>
        </div>
