    ...
    <section class="mt-30 w-full flex flex-col items-center gap-2">
        <div class="w-140 flex flex-col pb-10 pt-2 bg-[#2F2B2E] gap-10 rounded-lg">
            <div class="flex flex-row px-10 gap-4">
                ...
                <form id="post-form" class="w-full flex flex-row rounded-lg px-2 py-2 gap-2">
                    @csrf
        ...
        <div id="post-container" class="py-2 gap-4 flex flex-col">
            @foreach ($posts as $p)
                <x-post-card :post-user-id="$userId" :likes="$p->likes->count()" :comments="$p->comments->count()"
                    :user-id="$p->author->id" :post-id="$p->id" :author="$p->author->name" :content="$p->content"
                    :timestamp="$p->formatted_timestamp" />
            @endforeach
        </div>
    </section>
@endsection
...
@section('script')
    <script>
        ...
        const makeForm = (action = null, placeholder = null) => {
            ...
            csrfInput.name = '_token'; // Laravel uses "_token"
            ...
            // Styling
            form.classList.add('flex', 'flex-row', 'rounded-lg', 'px-2', 'py-2', 'gap-2', 'comment-form');
            textArea.classList.add('overflow-hidden', 'px-4', 'py-2', 'border', 'border-white', 'outline-none', 'bg-transparent', 'text-white', 'w-full', 'rounded-lg');
            button.classList.add('py-2', 'px-6', 'bg-amber-300', 'text-base', 'font-semibold', 'text-black', 'rounded-lg', 'h-fit');
            button.textContent = 'Posting';

            form.append(csrfInput, textArea, button);
            return form;
        };


        const showForm = (element) => {
            element.append(makeForm(action = '', placeholder = 'Tulis Komentar...'));
        }
        ...
        document.querySelectorAll('.comment').forEach((commentIcon) => {
            commentIcon.addEventListener('click', () => {
                const div = commentIcon.closest('.post-container');
